...
for epoch in range(0, max_epochs + 1, 1):
    ...
    print(
        epoch,
        " \t\t",
        numx.mean(ae.reconstruction_error(train_data)),
        " \t",
...
# Show samples
samples = vis.tile_matrix_rows(
    train_data[0:100].T, v1, v2, 10, 10, border_size=1, normalized=True
)
vis.imshow_matrix(samples, "Data samples")

# Show reconstruction
samples = vis.tile_matrix_rows(
    ae.decode(ae.encode(train_data[0:100])).T,
    v1,
    v2,
    10,
    10,
...
vis.imshow_filter_optimal_gratings(
    ae.w[:, 0:num_filters], opt_frq[0:num_filters], opt_ang[0:num_filters]
)

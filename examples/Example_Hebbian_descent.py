...
def train_Hebbian_descent_model(
    train_data, train_label, centered, act, epochs, epsilon, batch_size, weightdecay
):
    ...
    for e in range(epochs):
        for b in range(0, num_pattern, batch_size):
            ...
            input_point = np.copy(
                train_data[b : (b + batch_size), :].reshape(
                    batch_size, train_data.shape[1]
                )
            )
            output_point = np.copy(
                train_label[b : (b + batch_size), :].reshape(
            ...
            # Calculate output
            z = (
                np.dot(input_point - model.layers[0].offset, model.layers[0].weights)
                + model.layers[0].bias
            )
            ...
            model.layers[0].weights += (
                epsilon / batch_size * update_w_new
                - weightdecay * model.layers[0].weights
            )
...
mymodel, results = train_Hebbian_descent_model(

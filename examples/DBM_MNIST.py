...
wl1 = Weight_layer(input_dim=N, output_dim=M, initial_weights=0.01, dtype=numx.float64)
wl2 = Weight_layer(input_dim=M, output_dim=O, initial_weights=0.01, dtype=numx.float64)
...
l2 = Binary_layer(
    wl1, wl2, data=None, initial_bias="AUTO", initial_offsets="AUTO", dtype=numx.float64
)
...
l2.bias += numx.dot(0.0 - l1.offset, wl1.weights) + numx.dot(
    0.0 - l3.offset, wl2.weights.T
)
...
for epoch in range(0, max_epochs):
    ...
    print(
        numx.mean(numxExt.get_norms(wl1.weights)),
        "\t",
        numx.mean(numxExt.get_norms(wl2.weights)),
        "\t",
    )
    print(
        numx.mean(numxExt.get_norms(l1.bias)),
        "\t",
        numx.mean(numxExt.get_norms(l2.bias)),
        "\t",
    )
    print(
        numx.mean(numxExt.get_norms(l3.bias)),
        "\t",
        numx.mean(l1.offset),
        "\t",
        numx.mean(l2.offset),
...
VIS.imshow_matrix(
    VIS.tile_matrix_rows(
        wl1.weights, v11, v12, v21, v22, border_size=1, normalized=False
    ),
    "Weights 1",
)
VIS.imshow_matrix(
    VIS.tile_matrix_rows(
        numx.dot(wl1.weights, wl2.weights),
        v11,
        v12,
        v31,
        v32,
...
VIS.imshow_matrix(
    VIS.tile_matrix_columns(samples, v11, v12, 10, batch_size, 1, False), "Samples"
)

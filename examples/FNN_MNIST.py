...
for epoch in range(1, max_epochs + 1):
    ...
    print(
        numx.mean(
            npExt.compare_index_of_max(model.forward_propagate(train_data), train_label)
        ),
        "\t",
    )
    print(
        numx.mean(
            npExt.compare_index_of_max(model.forward_propagate(test_data), test_label)
        )
    )

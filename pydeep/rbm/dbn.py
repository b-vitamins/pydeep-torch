...
class DBN(StackOfBipartiteGraphs):
    ...
    def backward_propagate(self, output_data, sample=False):
        ...
        for l in range(len(self._layers), 0, -1):
            self.states[l - 1] = self._layers[l - 1].probability_v_given_h(
                self.states[l]
            )
            if sample is True:
    ...
    def reconstruct(self, input_data, sample=False):
        ...
        return self.backward_propagate(
            self.forward_propagate(input_data, sample), sample
        )
    ...
    def sample_top_layer(self, sampling_steps=100, initial_state=None, sample=True):
        ...
        for s in range(sampling_steps):
            self.states[len(self._layers) - 1] = self._layers[
                len(self._layers) - 1
            ].probability_v_given_h(self.states[len(self._layers)])
            if sample is True:
                self.states[len(self._layers) - 1] = self._layers[
                    len(self._layers) - 1
                ].sample_v(self.states[len(self._layers) - 1])
            self.states[len(self._layers)] = self._layers[
                len(self._layers) - 1
            ].probability_h_given_v(self.states[len(self._layers) - 1])
            if sample is True:
                self.states[len(self._layers)] = self._layers[
                    len(self._layers) - 1
                ].sample_h(self.states[len(self._layers)])
        return self.states[len(self._layers)]

    def reconstruct_sample_top_layer(
        self, input_data, sampling_steps=100, sample_forward_backward=False
    ):
        ...
        self.backward_propagate(
            self.states[len(self.states) - 1], sample_forward_backward
        )
        return self.states[0]

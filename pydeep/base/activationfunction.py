...
class LeakyRectifier(Rectifier):
    ...
    def f(self, x):
        ...
        """
        return x * (
            (x < 0) * (self.negativeSlope - self.positiveSlope) + self.positiveSlope
        )
...
class SoftMax(object):
    ...
    def df(cls, x):
        ...
        :rtype: scalar or numpy array with the same shape as x.
        """
        result = x[0] * numx.eye(x.shape[1], x.shape[1]) - numx.dot(
            x[0].reshape(x.shape[1], 1), x[0].reshape(1, x.shape[1])
        ).reshape(1, x.shape[1], x.shape[1])
        for i in range(1, x.shape[0], 1):
            result = numx.vstack(
                (
                    result,
                    x[i] * numx.eye(x.shape[1], x.shape[1])
                    - numx.dot(
                        x[i].reshape(x.shape[1], 1), x[i].reshape(1, x.shape[1])
                    ).reshape(1, x.shape[1], x.shape[1]),
                )
            )
...
class KWinnerTakeAll(object):
    ...
    def f(self, x):
        ...
        if self.axis == 0:
            winner = numx.float64(
                act >= numx.atleast_2d(numx.sort(act, axis=self.axis)[-self.k, :])
            )
        else:
            winner = numx.float64(
                act.T >= numx.atleast_2d(numx.sort(act, axis=self.axis)[:, -self.k])
            ).T

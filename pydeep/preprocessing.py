...
class PCA(STANDARIZER):
    ...
    def train(self, data):
        ...
        if self.whiten is True:
            self.unprojection_matrix = (
                self.projection_matrix * numx.sqrt(self.eigen_values)
            ).T
            self.projection_matrix = self.projection_matrix / numx.sqrt(
                self.eigen_values
            )
    ...
    def unproject(self, data, num_components=None):
        ...
        if num_components is not None:
            n = num_components
        return (
            numx.dot(data, self.unprojection_matrix[0 : data.shape[1], 0:n])
            + self.mean[:, 0:n]
        )
...
class ZCA(PCA):
    ...
    def train(self, data):
        ...
        self.unprojection_matrix = numx.dot(
            self.unprojection_matrix.T * numx.sqrt(self.eigen_values),
            self.unprojection_matrix,
        )
...
class ICA(PCA):
    ...
    def train(self, data, iterations=1000, convergence=0.0, status=False):
        ...
        for epoch in range(0, iterations):
            ...
            self.projection_matrix = (
                numx.dot(data.T, hyptan) / data.shape[0]
                - numx.array(
                    numx.dot(
                        numx.ones((data.shape[1], 1)),
            ...
            self.projection_matrix = numx.dot(
                self.projection_matrix,
                numx.real(numx.dot(numx.dot(ev, numx.diag(ew) ** 0.5), ev.T)),
            )

            angle = numx.mean(
                numx.diagonal(
                    npext.angle_between_vectors(
                        projection_matrix_old.T, self.projection_matrix.T, True
                    )
                )
            )
    ...
    def log_likelihood(self, data):
        ...
        return numx.sum(
            numx.log(
                0.5 / (numx.cosh(numx.dot(self.unprojection_matrix, data.T)) ** 2.0)
            ),
            axis=0,
        ) + numx.log(numx.abs(numx.linalg.det(self.projection_matrix)))

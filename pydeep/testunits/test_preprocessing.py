...
from pydeep.preprocessing import (
    binarize_data,
    rescale_data,
    remove_rows_means,
    remove_cols_means,
...
class Test_Preprocessing(unittest.TestCase):
    ...
    def test_PCA(self):
        ...
        model.train(data)
        assert numx.all(
            numx.abs(data - model.unproject(model.project(data))) < self.epsilon
        )
    ...
    def test_ICA(self):
        ...
        for _ in range(100):
            ...
            assert numx.all(
                numx.abs(data_zca - model.unproject(model.project(data_zca)))
                < self.epsilon
            )
            ...
            res = model.projection_matrix / get_norms(
                model.projection_matrix, axis=None
            )
            v1 = res
            ...
            v1d = numx.max(
                [
                    angle_between_vectors(v1[0], mixMat_zca[0]),
                    angle_between_vectors(v1[1], mixMat_zca[1]),
                ]
            )
            v2d = numx.max(
                [
                    angle_between_vectors(v2[0], mixMat_zca[0]),
                    angle_between_vectors(v2[1], mixMat_zca[1]),
                ]
            )
            v3d = numx.max(
                [
                    angle_between_vectors(v3[0], mixMat_zca[0]),
                    angle_between_vectors(v3[1], mixMat_zca[1]),
                ]
            )
            v4d = numx.max(
                [
                    angle_between_vectors(v4[0], mixMat_zca[0]),
                    angle_between_vectors(v4[1], mixMat_zca[1]),
                ]
            )
            v5d = numx.max(
                [
                    angle_between_vectors(v5[0], mixMat_zca[0]),
                    angle_between_vectors(v5[1], mixMat_zca[1]),
                ]
            )
            v6d = numx.max(
                [
                    angle_between_vectors(v6[0], mixMat_zca[0]),
                    angle_between_vectors(v6[1], mixMat_zca[1]),
                ]
            )
            v7d = numx.max(
                [
                    angle_between_vectors(v7[0], mixMat_zca[0]),
                    angle_between_vectors(v7[1], mixMat_zca[1]),
                ]
            )
            v8d = numx.max(
                [
                    angle_between_vectors(v8[0], mixMat_zca[0]),
                    angle_between_vectors(v8[1], mixMat_zca[1]),
                ]
            )

...
class Test_FNN_model(unittest.TestCase):
    ...
    def test_forward_propagate(self):
        ...
        assert numx.all(
            numx.abs(target - model.forward_propagate(numx.array([0.1, 3.0, 5.0, 9.0])))
            < self.epsilon
        )
        assert numx.all(
            numx.abs(
                target - model.forward_propagate(numx.array([[0.1, 3.0, 5.0, 9.0]]))
            )
            < self.epsilon
        )
        print("successfully passed!")
    ...
    def normal(self, delta=1e-4):
        sys.stdout.write(
            "FNN_layer -> Performing finite difference check for 3 layer normal FNN  ... "
        )
        sys.stdout.flush()
        ...
        for c in costs:
            for a3 in acts3:
                for a2 in acts2:
                    for a1 in acts1:
                        if (
                            (c != CFct.CrossEntropyError and c != CFct.NegLogLikelihood)
                            or (
                                c == CFct.CrossEntropyError
                                and (a3 == AFct.Sigmoid or a3 == AFct.SoftMax)
                            )
                            or (c == CFct.NegLogLikelihood and a3 == AFct.SoftMax)
                        ):
                            res = self.check(
                                data=data,
                                delta=delta,
    ...
    def local(self, delta=1e-4):
        sys.stdout.write(
            "FNN_layer -> Performing finite difference check for 3 layer local FNN  ... "
        )
        sys.stdout.flush()
        data = numx.random.rand(1, 6 * 6)
        label = numx.zeros((1, 6 * 6))
    ...
    def sparse(self, delta=1e-4):
        sys.stdout.write(
            "FNN_layer -> Performing finite difference check for 3 layer sparse FNN  ... "
        )
        sys.stdout.flush()
        data = numx.random.rand(1, 6 * 6)
        label = numx.zeros((1, 6 * 6))
    ...
    def target(self, delta=1e-4):
        sys.stdout.write(
            "FNN_layer -> Performing finite difference check for 3 layer FNN all layers with targets ... "
        )
        sys.stdout.flush()
        delta = 1e-4
        data = numx.random.rand(1, 6 * 6)

...
class Test_SAE_Model(unittest.TestCase):
    ...
    def test_forward(self):
        ...
        assert (
            numx.sum(
                numx.abs(
                    Test_SAE_Model.stack.forward_propagate(numx.array([[1, 2], [3, 4]]))
                    - Test_SAE_Model.forward_target
                )
            )
            < 0.000001
        )
        ...
        assert (
            numx.sum(
                numx.abs(
                    ...
                    - Test_SAE_Model.backward_target
                )
            )
            < 0.000001
        )

...
def compare_BipartiteGraph_after_adding(
    ...
    if check_oldpart:
        ...
        assert numx.all(new_weights[pos + num_new_units :] == old_weights[pos:])
        assert numx.all(new_bias[pos + num_new_units :] == old_bias[pos:])
        assert numx.all(new_offset[pos + num_new_units :] == old_offset[pos:])
...
class Test_BipartiteGraph(unittest.TestCase):
    def test___init__(self):
        ...
        numx.random.seed(42)
        initial_weights = numx.random.randn()
        initial_visible_bias = numx.random.randn() * numx.ones((1, number_visibles))
        initial_hidden_bias = numx.random.randn() * numx.ones((1, number_hiddens))
        initial_visible_offsets = numx.random.randn() * numx.ones((1, number_visibles))
        initial_hidden_offsets = numx.random.randn() * numx.ones((1, number_hiddens))
        initial_weights = (
            numx.random.randn(number_visibles, number_hiddens) * initial_weights
        )
        ...
        numx.random.seed(42)
        initial_weights = (
            2.0 * numx.random.rand(number_visibles, number_hiddens) - 1.0
        ) * (4.0 * numx.sqrt(6.0 / (number_visibles + number_hiddens)))
        ...
        initial_visible_bias = (
            Sigmoid()
            .g(numx.clip(test_data_mean, 0.001, 0.9999))
            .reshape(model.ov.shape)
        )
        ...
        initial_visible_offsets = numx.random.randn() * numx.ones((1, number_visibles))
        initial_hidden_offsets = numx.random.randn() * numx.ones((1, number_hiddens))
        initial_visible_bias = numx.array(
            Sigmoid().g(numx.clip(initial_visible_offsets, 0.001, 0.9999))
        ).reshape(1, number_visibles)
        initial_hidden_bias = numx.array(
            Sigmoid().g(numx.clip(initial_hidden_offsets, 0.001, 0.9999))
    ...
    def test_update_offsets(self):
        ...
        assert numx.all(
            bpgraph.bv == old_bv + numx.dot(old_oh, bpgraph.w.T) * offset_ratio
        )
        assert numx.all(
            bpgraph.bh == old_bh + numx.dot(old_ov, bpgraph.w) * offset_ratio
        )
        print(" successfully passed!")
        sys.stdout.flush()
...
class Test_StackOfBipartiteGraphs(unittest.TestCase):
    ...
    def test_forward_backward_reconstruct(self):
        ...
        backward_target = numx.array(
            [[0.89076868, 0.78815199], [0.93154994, 0.82386556]]
        )
        rec_target = numx.array([[0.85960142, 0.74341554], [0.86000094, 0.74392953]])

        assert numx.sum(
            numx.abs(
                stack.forward_propagate(numx.array([[1, 2], [3, 4]])) - forward_target
            )
            < 0.000001
        )
        assert numx.sum(
            numx.abs(
                stack.backward_propagate(numx.array([[1, 2], [3, 4]])) - backward_target
            )
            < 0.000001
        )
        assert numx.sum(
        ...
        assert numx.sum(
            numx.abs(stack.reconstruct(numx.array([[1, 2], [3, 4]])) - rec_target)
            < 0.000001
        )

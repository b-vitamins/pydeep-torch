...
class TestEstimator(unittest.TestCase):
    ...
    def test_reconstruction_error(self):
        ...
        sys.stdout.flush()
        numx.random.seed(42)
        rec = Estimator.reconstruction_error(
            self.bbrbm,
            self.bbrbmData,
            k=1,
            beta=1.0,
        ...
        rec = Estimator.reconstruction_error(
            self.bbrbm, testList, k=10, beta=1.0, use_states=False, absolut_error=False
        )
        assert numx.all(numx.abs(rec) < self.epsilon)
    ...
    def test_log_likelihood_v(self):
        ...
        ll = numx.mean(
            Estimator.log_likelihood_v(
                self.bbrbm, self.bbrbmTruelogZ, self.bbrbmData, 1.0
            )
        )
        ...
        for i in range(self.bbrbmData.shape[0]):
            testList.append(self.bbrbmData[i].reshape(1, 4))
        ll = numx.mean(
            Estimator.log_likelihood_v(self.bbrbm, self.bbrbmTruelogZ, testList, 1.0)
        )
        assert numx.all(numx.abs(ll - self.bbrbmTrueLL) < self.epsilon)
        print(" successfully passed!")
        sys.stdout.flush()
    ...
    def test_log_likelihood_h(self):
        ...
        ll = numx.mean(
            Estimator.log_likelihood_h(self.bbrbm, self.bbrbmTruelogZ, hdata, 1.0)
        )
    ...
    def test_partition_function_factorize_v(self):
        ...
        LogZ = Estimator.partition_function_factorize_v(
            self.bbrbm, beta=None, batchsize_exponent=0, status=False
        )
        assert numx.all(numx.abs(LogZ - self.bbrbmTruelogZ) < self.epsilon)
        LogZ = Estimator.partition_function_factorize_v(
            self.bbrbm, beta=None, batchsize_exponent=3, status=False
        )
        assert numx.all(numx.abs(LogZ - self.bbrbmTruelogZ) < self.epsilon)
        LogZ = Estimator.partition_function_factorize_v(
            self.bbrbm, beta=None, batchsize_exponent=555, status=False
        )
        assert numx.all(numx.abs(LogZ - self.bbrbmTruelogZ) < self.epsilon)
        print(" successfully passed!")
        sys.stdout.flush()
    ...
    def test_annealed_importance_sampling(self):
        ...
        sys.stdout.flush()
        numx.random.seed(42)
        LogZ = Estimator.annealed_importance_sampling(
            self.bbrbm, num_chains=100, k=1, betas=100, status=False
        )
        assert numx.all(numx.abs(LogZ[0] - self.bbrbmTruelogZ) < 0.5)
        LogZ = Estimator.annealed_importance_sampling(
            self.bbrbm, num_chains=100, k=1, betas=1000, status=False
        )
        assert numx.all(numx.abs(LogZ[0] - self.bbrbmTruelogZ) < 0.05)
        LogZ = Estimator.annealed_importance_sampling(
            self.bbrbm, num_chains=100, k=1, betas=10000, status=False
        )
    ...
    def test_reverse_annealed_importance_sampling(self):
        ...
        sys.stdout.flush()
        numx.random.seed(42)
        LogZ = Estimator.reverse_annealed_importance_sampling(
            self.bbrbm, num_chains=100, k=1, betas=100, status=False
        )
        assert numx.all(numx.abs(LogZ[0] - self.bbrbmTruelogZ) < 0.5)
        LogZ = Estimator.reverse_annealed_importance_sampling(

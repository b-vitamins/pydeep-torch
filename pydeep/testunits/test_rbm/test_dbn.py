...
class TestDBNModel(unittest.TestCase):
    ...
    def test_forward_propagate(self):
        sys.stdout.write(
            "Deep Believe Network -> Performing forward_propagate test ..."
        )
        sys.stdout.flush()
        ...
        assert (
            numx.sum(
                numx.abs(
                    TestDBNModel.stack.forward_propagate(numx.array([[1, 0], [0, 1]]))
                    - forward_target
                )
            )
            < 0.000001
        ...
        assert (
            numx.sum(
                numx.abs(
                    TestDBNModel.stack.forward_propagate(
                        numx.array([[1, 0], [0, 1]]), True
                    )
                    - forward_target
                )
            )
    ...
    def test_backward_propagate(self):
        sys.stdout.write(
            "Deep Believe Network -> Performing backward_propagate test ..."
        )
        sys.stdout.flush()

        backward_target = numx.array(
            [[0.30266536, 0.52656316], [0.69733464, 0.47343684]]
        )
        assert (
            numx.sum(
                numx.abs(
                    TestDBNModel.stack.backward_propagate(numx.array([[1, 0], [0, 1]]))
                    - backward_target
                )
            )
            < 0.000001
        ...
        assert (
            numx.sum(
                numx.abs(
                    TestDBNModel.stack.backward_propagate(
                        numx.array([[1, 0], [0, 1]]), True
                    )
                    - backward_target
                )
    ...
    def test_reconstruct(self):
        ...
        assert (
            numx.sum(
                numx.abs(
                    TestDBNModel.stack.reconstruct(numx.array([[1, 0], [0, 1]]))
                    - rec_target
                )
            )
            < 0.000001
        ...
        assert (
            numx.sum(
                numx.abs(
                    TestDBNModel.stack.reconstruct(numx.array([[1, 0], [0, 1]]), True)
                    - rec_target
                )
            )
            < 0.000001
    ...
    def test_reconstruct_sample_top_layer(self):
        sys.stdout.write(
            "Deep Believe Network -> Performing reconstruct_sample_top_layer test ..."
        )
        sys.stdout.flush()
        ...
        assert (
            numx.sum(
                numx.abs(
                    TestDBNModel.stack.reconstruct_sample_top_layer(
    ...
    def test_sample_top_layer(self):
        ...
        assert (
            numx.sum(
                numx.abs(
                    TestDBNModel.stack.sample_top_layer(1, sample=False)
                    - numx.array([[0.81216745, 0.80223488], [0.05331971, 0.94985651]])
                )
            )
            < 0.000001
        )
        ...
        assert (
            numx.sum(
                numx.abs(
                    TestDBNModel.stack.sample_top_layer(100, sample=True)
                    - numx.array([[0, 0], [0, 1]])
                )
            )
            < 0.000001
        ...
        assert (
            numx.sum(
                numx.abs(
                    TestDBNModel.stack.sample_top_layer(
                        100, initial_state=numx.array([[1, 1], [1, 1]]), sample=True
                    )
                    - numx.array([[0, 0], [0, 1]])
                )
            )

...
class TestTrainer(unittest.TestCase):
    ...
    def test__calculate_centered_gradient(self):
        sys.stdout.write(
            "RBM Trainer -> Performing calculate_centered_gradient test ... "
        )
        sys.stdout.flush()
        numx.random.seed(42)
        gradsOrg = [
            numx.array([[0.3, 0.5], [-0.4, 0.8]]),
            numx.array([[2.0, 4.0]]),
            numx.array([[-2.0, -1.0]]),
        ]
        visible_offsets = numx.array([[0.0, 0.0]])
        hidden_offsets = numx.array([[0.0, 0.0]])
        gradsCen = Trainer.CD._calculate_centered_gradient(
            gradsOrg, visible_offsets, hidden_offsets
        )
        ...
        target = [
            numx.array([[1.1, 0.6], [0.2, 0.5]]),
            numx.array([[1.77, 3.88]]),
            numx.array([[-2.65, -1.55]]),
        ]
        gradsCen = Trainer.CD._calculate_centered_gradient(
            gradsOrg, visible_offsets, hidden_offsets
        )
    ...
    def test___init__(self):
        ...
        assert numx.all(
            numx.abs(
                trainer.visible_offsets - data.mean(axis=0).reshape(1, data.shape[1])
            )
            < self.epsilon
        )
    ...
    def test_adapt_gradient(self):
        ...
        target = [
            ((pos_gradients[0] - neg_gradients[0]) / batch_size),
            ((pos_gradients[1] - neg_gradients[1]) / batch_size),
            ((pos_gradients[2] - neg_gradients[2]) / batch_size),
        ]
        assert numx.all(
            numx.abs(trainer.parameter_updates[0] - target[0]) < self.epsilon
        )
        assert numx.all(
            numx.abs(trainer.parameter_updates[1] - target[1]) < self.epsilon
        )
        assert numx.all(
            numx.abs(trainer.parameter_updates[2] - target[2]) < self.epsilon
        )
        ...
        target = [
            numx.array([[-1.22862968, 1.4092448], [2.05873296, 2.15099481]]),
            numx.array([[1.0, 5.0]]),
            numx.array([[-3.0, 3.0]]),
        ]
        assert numx.all(
        ...
        target = [
            numx.array([[0.4, 0.0], [0.5, 0.6]]),
            numx.array([[1.0, 5.0]]),
            numx.array([[-3.59, 2.31]]),
        ]
        assert numx.all(
            numx.abs(trainer.parameter_updates[0] - target[0]) < self.epsilon
        )
        ...
        target = [
            numx.array([[2.5, -2.6], [0.5, -1.9]]),
            numx.array([[0.77, 5.23]]),
            numx.array([[-5.25, 6.03]]),
        ]
        assert numx.all(
            numx.abs(trainer.parameter_updates[0] - target[0]) < self.epsilon
        )
    ...
    def test__train(self):
        ...
        data = generate_bars_and_stripes_complete(2)
        data = numx.vstack((data[0], data, data[5]))
        model = Model.BinaryBinaryRBM(
            4, 2, initial_visible_offsets=0.0, initial_hidden_offsets=0.0
        )
        trainer = Trainer.CD(model, data)
        trainer.train(
            data=data,
    ...
    def test_GD(self):
        ...
        data = numx.vstack((data[0], data, data[5]))
        model = Model.BinaryBinaryRBM(
            4, 4, data, initial_visible_offsets=0.0, initial_hidden_offsets=0.0
        )
        ...
        LL = numx.mean(
            Estimator.log_likelihood_v(
                model,
                Estimator.partition_function_factorize_h(model, batchsize_exponent=3),
                data,
            )
        )

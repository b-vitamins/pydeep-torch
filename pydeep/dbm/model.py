...
class DBM_model(object):
    ...
    def meanfield(self, chains, k, fixed_layer=None, inplace=True):
        ...
        for i in range(self.num_layers):
            if fixed_layer[i] is True:
                if i > 0:
                    pre_top[i - 1] = self.layers[i].input_weight_layer.propagate_down(
                        chains[i]
                    )
                if i < self.num_layers - 1:
                    pre_bottom[i + 1] = self.layers[i].output_weight_layer.propagate_up(
                        chains[i]
                    )
        ...
        for _ in range(k):
            for i in range(1, self.num_layers - 1, 2):
                if fixed_layer[i] is False:
                    target[i] = self.layers[i].activation(
                        target[i - 1], target[i + 1], pre_bottom[i], pre_top[i]
                    )[0]
            # Top layer if even
            if self.even_num_layers is True:
                if fixed_layer[self.num_layers - 1] is False:
                    target[self.num_layers - 1] = self.layers[
                        self.num_layers - 1
                    ].activation(
                        target[self.num_layers - 2],
                        None,
                        pre_bottom[self.num_layers - 1],
                        None,
                    )[0]
            # First Layer
            if fixed_layer[0] is False:
                target[0] = self.layers[0].activation(
                    None, target[1], None, pre_top[0]
                )[0]
    ...
    def sample(self, chains, k, fixed_layer=None, inplace=True):
        ...
        for _ in range(k):
            ...
            if self.even_num_layers is True:
                if fixed_layer[self.num_layers - 1] is False:
                    ...
                    target[self.num_layers - 1] = self.layers[
                        ...
                        self.layers[self.num_layers - 1].activation(
                            target[self.num_layers - 2],
                            None,
                            pre_bottom[self.num_layers - 1],
                            None,
            ...
            if fixed_layer[0] is False:
                ...
                target[0] = self.layers[0].sample(
                    self.layers[0].activation(None, target[1], None, pre_top[0])
                )
            # else:
    ...
    def update(
        self, chains_d, chains_m, lr_W, lr_b, lr_o, restriction=None, restrict_typ=None
    ):
        ...
        for l in range(1, self.num_layers):
            grad = self.layers[l].input_weight_layer.calculate_weight_gradients(
                chains_d[l - 1],
                chains_d[l],
                chains_m[l - 1],
                chains_m[l],
            ...
            self.layers[l].input_weight_layer.update_weights(
                lr_W[l - 1] * grad, restriction, restrict_typ
            )
            grad_w.append(grad)

        for l in range(self.num_layers):
            if l == 0:
                grad_b = self.layers[0].calculate_gradient_b(
                    chains_d[0],
                    chains_m[0],
                    None,
                    self.layers[1].offset,
                    None,
                    grad_w[0],
                )
            elif l == self.num_layers - 1:
                grad_b = self.layers[l].calculate_gradient_b(
                    chains_d[l],
                    chains_m[l],
                    self.layers[l - 1].offset,
                    None,

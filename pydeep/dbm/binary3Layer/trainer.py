...
class CD(object):
    ...
    def train(self, data, epsilon, k=[3, 1], offset_typ="DDD", meanfield=False):
        ...
        if meanfield == False:
            for _ in range(k[0]):
                d2 = Sigmoid.f(
                    id1 + numx.dot(d3 - self.model.o3, self.model.W2.T) + self.model.b2
                )
                d2 = self.model.dtype(d2 > numx.random.random(d2.shape))
                d3 = Sigmoid.f(
                    numx.dot(d2 - self.model.o2, self.model.W2) + self.model.b3
                )
            ...
            if meanfield == True:
                ...
                d3_new = Sigmoid.f(
                    numx.dot(d2_new - self.model.o2, self.model.W2) + self.model.b3
                )
                while (
                    numx.max(numx.abs(d2_new - d2)) > meanfield
                    or numx.max(numx.abs(d3_new - d3)) > meanfield
                ):
                    d2 = d2_new
                    d3 = d3_new
                    d2_new = Sigmoid.f(
        ...
        self.model.b2 += epsilon[5] * numx.dot(
            new_o1 - self.model.o1, self.model.W1
        ) + epsilon[7] * numx.dot(new_o3 - self.model.o3, self.model.W2.T)
        ...
        # Calculate gradients
        dW1 = numx.dot((data - self.model.o1).T, d2 - self.model.o2) - numx.dot(
...
class PCD_check(object):
    ...
    def train(self, data, epsilon, k=[3, 1], offset_typ="DDD", meanfield=False):
        ...
        for _ in range(k[1]):
            ...
            self.m1 = Sigmoid.f(
                numx.dot(self.m2 - self.model.o2, self.model.W1.T) + self.model.b1
            )
            self.m1 = self.model.dtype(self.m1 > numx.random.random(self.m1.shape))
            self.m3 = Sigmoid.f(
                numx.dot(self.m2 - self.model.o2, self.model.W2) + self.model.b3
            )

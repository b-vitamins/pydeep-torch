...
class BinaryBinaryDBM(object):
    def __init__(
        self, input_dim, hidden1_dim, hidden2_dim, offset_typ, data, dtype=numx.float64
    ):
    ...
    def energy(self, x, h1, h2):
        ...
        # Caluclate energy
        return (
            -numx.dot(xtemp, self.b1.T)
            - numx.dot(h1temp, self.b2.T)
            - numx.dot(h2temp, self.b3.T)
            - numx.sum(numx.dot(xtemp, self.W1) * h1temp, axis=1).reshape(
                h1temp.shape[0], 1
            )
            - numx.sum(numx.dot(h1temp, self.W2) * h2temp, axis=1).reshape(
                h2temp.shape[0], 1
            )
        )
    ...
    def unnormalized_log_probability_h1(self, h1):
        ...
        factorx = numx.sum(
            numx.log(
                numx.exp(activation * (1.0 - self.o1)) + numx.exp(-activation * self.o1)
            ),
            axis=1,
        ).reshape(temp.shape[0], 1)
        # Value for h1 via factorization over h2
        activation = numx.dot(temp, self.W2) + self.b3
        factorh2 = numx.sum(
            numx.log(
                numx.exp(activation * (1.0 - self.o3)) + numx.exp(-activation * self.o3)
            ),
            axis=1,
        ).reshape(temp.shape[0], 1)

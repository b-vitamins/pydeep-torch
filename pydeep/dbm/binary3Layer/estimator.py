...
def _partition_function_exact_check(model, batchsize_exponent="AUTO"):
    ...
    for batch in range(1, num_batches + 1):
        ...
        bitCombinations = npExt.generate_binary_code(
            bit_length, batchsize_exponent, batch - 1
        )
        # calculate LL
        log_prob_vv_all[(batch - 1) * batchSize : batch * batchSize] = (
            model.unnormalized_log_probability_h1(bitCombinations).reshape(
...
def partition_function_AIS(model, num_chains=100, k=1, betas=10000, status=False):
    ...
    return RBM_ESTIMATOR.annealed_importance_sampling(
        model=rbm, num_chains=num_chains, k=k, betas=betas, status=status
    )
...
def _LL_lower_bound_check(model, x, lnZ, conv_thres=0.0001, max_iter=100000):
    ...
    for i in range(x.shape[0]):
        ...
        d2_temp = 0.0
        d2_new = Sigmoid.f(
            id1[i, :] + numx.dot(d3_temp - model.o3, model.W2.T) + model.b2
        )
        d3_new = Sigmoid.f(numx.dot(d2_new - model.o2, model.W2) + model.b3)
        while (
            numx.max(numx.abs(d2_new - d2_temp)) > conv_thres
            or numx.max(numx.abs(d3_new - d3_temp)) > conv_thres
        ):
            d2_temp = d2_new
            d3_temp = d3_new
            d2_new = Sigmoid.f(
                id1[i, :] + numx.dot(d3_new - model.o3, model.W2.T) + model.b2
            )
            d3_new = Sigmoid.f(numx.dot(d2_new - model.o2, model.W2) + model.b3)
        d2[i] = numx.clip(d2_new, 0.0000000000000001, 0.9999999999999999).reshape(
            1, model.hidden1_dim
        )
        d3[i] = numx.clip(d3_new, 0.0000000000000001, 0.9999999999999999).reshape(
            1, model.hidden2_dim
        )
    # Return ernegy of states + the entropy of h1.h2 due to the mean field approximation
    return (
        -model.energy(x, d2, d3)
        - lnZ
        - numx.sum(d2 * numx.log(d2) + (1.0 - d2) * numx.log(1.0 - d2), axis=1).reshape(
            x.shape[0], 1
        )
        - numx.sum(d3 * numx.log(d3) + (1.0 - d3) * numx.log(1.0 - d3), axis=1).reshape(
            x.shape[0], 1
        )
    )
...
def LL_lower_bound(model, x, lnZ, conv_thres=0.0001, max_iter=1000):
    ...
    e2 = numx.sum(
        numx.log(
            numx.exp(-(numx.dot(h1temp, model.W2) + model.b3) * (model.o3))
            + numx.exp((numx.dot(h1temp, model.W2) + model.b3) * (1.0 - model.o3))
        ),
        axis=1,
    ).reshape(x.shape[0], 1)
    e1 = (
        numx.dot(xtemp, model.b1.T)
        + numx.dot(h1temp, model.b2.T)
        + numx.sum(numx.dot(xtemp, model.W1) * h1temp, axis=1).reshape(
            h1temp.shape[0], 1
        )
        + e2
    )

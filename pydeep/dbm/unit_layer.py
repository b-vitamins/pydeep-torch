...
class Binary_layer(object):
    def __init__(
        ...
        if data is not None:
            ...
            if self.input_dim != data.shape[1]:
                raise ValueError(
                    "Data dimension and model input \
    ...
    def activation(
        self, bottom_up_states, top_down_states, bottom_up_pre=None, top_down_pre=None
    ):
    ...
    def update_biases(self, bias_updates, restriction, restriction_typ):
        ...
        if numx.isscalar(restriction):
            if restriction > 0:
                if (
                    restriction_typ is "Cols"
                    or restriction_typ is "Rows"
                    or restriction_typ is "Mat"
                ):
                    bias_updates = numxExt.restrict_norms(bias_updates, restriction)
    ...
    def update_offsets(self, offset_updates, shifting_factor):
        ...
        self.offset = (
            1.0 - shifting_factor
        ) * self.offset + shifting_factor * offset_updates
...
class Softmax_layer(Binary_layer):
    ...
    def activation(
        ...
        if self.input_weight_layer is not None:
            if bottom_up_pre is None:
                pre_act = (
                    self.input_weight_layer.propagate_up(bottom_up_states) + pre_act
                )
            else:
        ...
        if self.output_weight_layer is not None:
            if top_down_pre is None:
                pre_act = (
                    self.output_weight_layer.propagate_down(top_down_states) + pre_act
                )
            else:
                pre_act = top_down_pre + pre_act
        return numx.exp(
            pre_act - numxExt.log_sum_exp(pre_act, axis=1).reshape(pre_act.shape[0], 1)
        ), pre_act
    ...
    def sample(self, activation):
        ...
        """
        probs = activation[0] / numx.sum(activation[0], axis=1).reshape(
            activation[0].shape[0], 1
        )
...
class Gaussian_layer(Binary_layer):
    ...
    def sample(self, activation):
        ...
        """
        return activation[0] + numx.random.randn(
            activation[0].shape[0], activation[0].shape[1]
        )
...
class Rectifier_layer(Binary_layer):
    def __init__(
        ...
        """
        # Call constructor of superclass
        super(Rectifier_layer, self).__init__(
            input_weight_layer,
            output_weight_layer,
            data,
            initial_bias,
    ...
    def sample(self, activation):
        ...
        activation3 = numx.maximum(
            0.0,
            activation[1]
            + numx.random.randn(activation[1].shape[0], activation[1].shape[1])
            * numx.sqrt(Sigmoid.f(activation[1])),
        )
